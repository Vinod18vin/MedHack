...
// Global cache for tracking recently processed request IDs (for idempotency)
app.locals.requestCache = new Map();
const IDEMPOTENCY_WINDOW_MS = 5000; // Cache submissions for 5 seconds
...
/** ------------------------- Manual Appointment (IDEMPOTENCY CHECK ADDED) ------------------------- **/
app.post('/submitAppointment', async (req, res) => {
  const { submissionId, name, age, email, symptoms, doctor, mode, language } = req.body;
  
  if (!submissionId) {
      console.warn("Received submission without ID. Processing without idempotency check.");
  } else {
      // 1. Check if the submissionId is already in cache
      if (app.locals.requestCache.has(submissionId)) {
          console.warn(`Duplicate submission ID detected: ${submissionId}. Request IGNORED.`);
          // Return a 200 OK status to the client so the frontend cleans up as successful
          return res.json({ ok: true, message: 'Duplicate transaction ignored.' });
      }

      // 2. Add submissionId to cache and schedule cleanup
      app.locals.requestCache.set(submissionId, Date.now());
      setTimeout(() => {
          app.locals.requestCache.delete(submissionId);
